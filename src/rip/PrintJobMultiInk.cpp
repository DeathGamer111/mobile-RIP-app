    ...
    int xdpi, int ydpi, DirectPrintSpool& spool, bool includeFinalPrn)
{
    try {
        ...
        const auto strategy = BoundedRasterPipeline::selectRasterStrategy(
            outputSize.width(), outputSize.height(), logicalChannels, true);
        ...
        qInfo().noquote()
            << QStringLiteral(
                   "PrintJobMultiInk: raster backend path = %1; output %2 x %3; "
        ...
        if (useInMemory) {
            ...
            spool = {};
            qInfo().noquote()
                << "PrintJobMultiInk: raster backend path switched to bounded strips after the in-memory path did not complete.";
        }
